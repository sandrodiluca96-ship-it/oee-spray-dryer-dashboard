import streamlit as st
import pandas as pd
import plotly.graph_objects as go
import plotly.express as px
from pathlib import Path
from datetime import datetime
import math

st.set_page_config(page_title="OEE Spray Dryer", layout="wide")

PRODUZIONI_FILE = Path("data/produzioni.csv")
FERMI_FILE = Path("data/fermi.csv")
PRODOTTI_FILE = Path("data/prodotti.csv")

TURNO_H = 8

CAUSALI_FERMO = [
    "Attesa prodotto",
    "Attesa analisi",
    "Cambio lotto",
    "Lavaggio",
    "Guasto",
    "Manutenzione programmata",
    "Manutenzione straordinaria",
    "Pulizia",
    "Altro",
]

CAUSALI_TECNICHE = [
    "Guasto",
    "Manutenzione straordinaria",
]

CAUSE_NC = [
    "Titolo insufficiente",
    "Umidità alta",
    "Umidità bassa",
    "Granulometria fuori specifica",
    "Densità apparente",
    "Colore",
    "Odore",
    "Microbiologia",
    "Residui solventi",
    "Corpo estraneo",
    "Errore documentale",
    "Altro",
]

COL_PRODUZIONI = [
    "id_lotto",
    "data",
    "turno",
    "codice_prodotto",
    "descrizione_prodotto",
    "lotto",
    "kg_molle",
    "residuo_secco",
    "kg_polvere_teorica",
    "kg_polvere_reale",
    "ora_inizio",
    "ora_fine",
    "ore_produzione",
    "stato_analisi",
    "kg_conformi",
    "kg_nc",
    "causa_nc",
    "note_nc",
    "data_rilascio_analisi",
]

COL_FERMI = [
    "id_fermo",
    "data",
    "turno",
    "ora_inizio",
    "ora_fine",
    "durata_h",
    "causale_fermo",
    "note_fermo",
]

COL_PRODOTTI = [
    "codice_prodotto",
    "descrizione_prodotto",
    "capacita_kg_h",
]

def inizializza_file():
    Path("data").mkdir(parents=True, exist_ok=True)

    if not PRODUZIONI_FILE.exists():
        pd.DataFrame(columns=COL_PRODUZIONI).to_csv(PRODUZIONI_FILE, index=False)

    if not FERMI_FILE.exists():
        pd.DataFrame(columns=COL_FERMI).to_csv(FERMI_FILE, index=False)

    if not PRODOTTI_FILE.exists():
        pd.DataFrame(columns=COL_PRODOTTI).to_csv(PRODOTTI_FILE, index=False)

def leggi_csv(path, colonne):
    inizializza_file()
    df = pd.read_csv(path)
    for col in colonne:
        if col not in df.columns:
            df[col] = None
    return df[colonne]

def leggi_produzioni():
    return leggi_csv(PRODUZIONI_FILE, COL_PRODUZIONI)

def leggi_fermi():
    return leggi_csv(FERMI_FILE, COL_FERMI)

def leggi_prodotti():
    df = leggi_csv(PRODOTTI_FILE, COL_PRODOTTI)
    if df.empty:
        df = pd.DataFrame([{
            "codice_prodotto": "INSERIRE",
            "descrizione_prodotto": "Inserire anagrafica prodotti",
            "capacita_kg_h": 20,
        }])
    df["capacita_kg_h"] = pd.to_numeric(df["capacita_kg_h"], errors="coerce").fillna(20)
    return df

def safe_div(num, den):
    if den is None or den == 0 or pd.isna(den):
        return 0
    return num / den

def ore_da_orari(ora_inizio, ora_fine):
    if ora_inizio is None or ora_fine is None:
        return 0
    dt_inizio = datetime.combine(datetime.today(), ora_inizio)
    dt_fine = datetime.combine(datetime.today(), ora_fine)
    durata = (dt_fine - dt_inizio).total_seconds() / 3600
    if durata < 0:
        durata += 24
    return durata

def prepara_produzioni(df):
    if df.empty:
        return df

    numeric_cols = [
        "kg_molle",
        "residuo_secco",
        "kg_polvere_teorica",
        "kg_polvere_reale",
        "ore_produzione",
        "kg_conformi",
        "kg_nc",
    ]
    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)

    df["data"] = pd.to_datetime(df["data"], errors="coerce")
    df["kg_polvere_teorica"] = df["kg_molle"] * df["residuo_secco"] / 100
    df["resa"] = df.apply(lambda r: safe_div(r["kg_polvere_reale"], r["kg_polvere_teorica"]), axis=1)
    return df

def prepara_fermi(df):
    if df.empty:
        return df

    df["data"] = pd.to_datetime(df["data"], errors="coerce")
    df["durata_h"] = pd.to_numeric(df["durata_h"], errors="coerce").fillna(0)
    return df

def gauge_oee(valore):
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=valore * 100,
        number={"suffix": "%", "font": {"size": 42}},
        title={"text": "OEE Spray Dryer", "font": {"size": 24}},
        gauge={
            "axis": {"range": [0, 100]},
            "bar": {"thickness": 0.28},
            "steps": [
                {"range": [0, 50], "color": "#ff4b4b"},
                {"range": [50, 65], "color": "#ffa500"},
                {"range": [65, 75], "color": "#ffd966"},
                {"range": [75, 85], "color": "#9be564"},
                {"range": [85, 100], "color": "#2ecc71"},
            ],
            "threshold": {
                "line": {"color": "black", "width": 4},
                "thickness": 0.75,
                "value": 75
            },
        },
    ))
    fig.update_layout(height=330, margin=dict(l=30, r=30, t=70, b=20))
    return fig

def filtra_periodo(df, col_data, periodo):
    if df.empty or len(periodo) != 2:
        return df
    start = pd.to_datetime(periodo[0])
    end = pd.to_datetime(periodo[1])
    return df[(df[col_data] >= start) & (df[col_data] <= end)]

def calcola_cruscotto(prod, fermi):
    if prod.empty:
        return {
            "oee": 0,
            "disp_operativa": 0,
            "disp_tecnica": 0,
            "performance": 0,
            "quality": 0,
            "kg_reali": 0,
            "kg_conformi": 0,
            "kg_nc": 0,
            "kg_molle": 0,
            "kg_teorici": 0,
            "resa": 0,
            "tempo_pianificato": 0,
        }

    n_turni = prod[["data", "turno"]].drop_duplicates().shape[0]
    tempo_pianificato = n_turni * TURNO_H

    fermi_totali = fermi["durata_h"].sum() if not fermi.empty else 0
    fermi_tecnici = fermi[fermi["causale_fermo"].isin(CAUSALI_TECNICHE)]["durata_h"].sum() if not fermi.empty else 0

    disp_operativa = safe_div(tempo_pianificato - fermi_totali, tempo_pianificato)
    disp_tecnica = safe_div(tempo_pianificato - fermi_tecnici, tempo_pianificato)

    prodotti = leggi_prodotti()[["codice_prodotto", "capacita_kg_h"]]
    prod_perf = prod.merge(prodotti, on="codice_prodotto", how="left")
    prod_perf["capacita_kg_h"] = pd.to_numeric(prod_perf["capacita_kg_h"], errors="coerce").fillna(20)
    prod_perf["kg_teorici_macchina"] = prod_perf["capacita_kg_h"] * prod_perf["ore_produzione"]

    kg_reali = prod_perf["kg_polvere_reale"].sum()
    kg_teorici_macchina = prod_perf["kg_teorici_macchina"].sum()
    performance = safe_div(kg_reali, kg_teorici_macchina)

    prod_analizzati = prod[prod["stato_analisi"].isin(["CONFORME", "NON_CONFORME"])].copy()
    kg_conformi = prod_analizzati["kg_conformi"].sum() if not prod_analizzati.empty else 0
    kg_prodotti_analizzati = prod_analizzati["kg_polvere_reale"].sum() if not prod_analizzati.empty else 0
    quality = safe_div(kg_conformi, kg_prodotti_analizzati)

    oee = disp_operativa * performance * quality

    return {
        "oee": max(0, oee),
        "disp_operativa": max(0, disp_operativa),
        "disp_tecnica": max(0, disp_tecnica),
        "performance": performance,
        "quality": quality,
        "kg_reali": kg_reali,
        "kg_conformi": kg_conformi,
        "kg_nc": prod["kg_nc"].sum(),
        "kg_molle": prod["kg_molle"].sum(),
        "kg_teorici": prod["kg_polvere_teorica"].sum(),
        "resa": safe_div(kg_reali, prod["kg_polvere_teorica"].sum()),
        "tempo_pianificato": tempo_pianificato,
    }

inizializza_file()

st.title("Dashboard OEE Spray Dryer - V6")

if "salvato" not in st.session_state:
    st.session_state["salvato"] = False

if st.session_state["salvato"]:
    st.session_state["salvato"] = False
    st.success("Operazione salvata correttamente.")

tab_produzione, tab_fermi, tab_qualita, tab_cruscotto, tab_formule, tab_prodotti, tab_gestione = st.tabs([
    "Produzione",
    "Fermi",
    "Qualità",
    "Cruscotto OEE",
    "Assunzioni e formule",
    "Anagrafica prodotti",
    "Gestione dati",
])

with tab_produzione:
    st.subheader("Registrazione produzione lotto")

    prodotti = leggi_prodotti()
    prodotti["label"] = prodotti["codice_prodotto"].astype(str) + " - " + prodotti["descrizione_prodotto"].astype(str)

    with st.form("form_produzione", clear_on_submit=True):
        data = st.date_input("Data produzione")
        turno = st.selectbox("Turno", ["1", "2", "3"])
        prodotto_label = st.selectbox("Prodotto", prodotti["label"].tolist())
        prodotto = prodotti[prodotti["label"] == prodotto_label].iloc[0]

        codice_prodotto = prodotto["codice_prodotto"]
        descrizione_prodotto = prodotto["descrizione_prodotto"]

        st.info(f"Codice: {codice_prodotto} | Descrizione: {descrizione_prodotto}")

        lotto = st.text_input("Lotto")

        c1, c2 = st.columns(2)
        with c1:
            kg_molle = st.number_input("Kg estratto molle inserito", min_value=0.0, step=1.0)
        with c2:
            residuo_secco = st.number_input("Residuo secco %", min_value=0.0, max_value=100.0, step=0.1)

        kg_polvere_teorica = kg_molle * residuo_secco / 100
        st.metric("Kg polvere teorica", f"{kg_polvere_teorica:.2f} kg")

        kg_polvere_reale = st.number_input("Kg polvere prodotta", min_value=0.0, step=1.0)

        c3, c4 = st.columns(2)
        with c3:
            ora_inizio = st.time_input("Ora inizio produzione")
        with c4:
            ora_fine = st.time_input("Ora fine produzione")

        ore_produzione = ore_da_orari(ora_inizio, ora_fine)
        st.metric("Ore produzione calcolate", f"{ore_produzione:.2f} h")

        salva_produzione = st.form_submit_button("Salva produzione")

    if salva_produzione:
        if lotto.strip() == "":
            st.error("Inserisci il lotto.")
        elif ore_produzione <= 0:
            st.error("Controlla ora inizio e ora fine produzione.")
        else:
            df = leggi_produzioni()
            duplicato = df[
                (df["data"].astype(str) == str(data)) &
                (df["turno"].astype(str) == str(turno)) &
                (df["codice_prodotto"].astype(str) == str(codice_prodotto)) &
                (df["lotto"].astype(str) == str(lotto))
            ]

            if not duplicato.empty:
                st.warning("Produzione già presente con stessa data, turno, prodotto e lotto.")
            else:
                nuova = pd.DataFrame([{
                    "id_lotto": datetime.now().strftime("%Y%m%d%H%M%S"),
                    "data": data,
                    "turno": turno,
                    "codice_prodotto": codice_prodotto,
                    "descrizione_prodotto": descrizione_prodotto,
                    "lotto": lotto,
                    "kg_molle": kg_molle,
                    "residuo_secco": residuo_secco,
                    "kg_polvere_teorica": kg_polvere_teorica,
                    "kg_polvere_reale": kg_polvere_reale,
                    "ora_inizio": str(ora_inizio),
                    "ora_fine": str(ora_fine),
                    "ore_produzione": ore_produzione,
                    "stato_analisi": "ATTESA_ANALISI",
                    "kg_conformi": 0,
                    "kg_nc": 0,
                    "causa_nc": "",
                    "note_nc": "",
                    "data_rilascio_analisi": "",
                }])
                df = pd.concat([df, nuova], ignore_index=True)
                df.to_csv(PRODUZIONI_FILE, index=False)
                st.session_state["salvato"] = True
                st.rerun()

with tab_fermi:
    st.subheader("Registrazione evento fermo")

    with st.form("form_fermi", clear_on_submit=True):
        data_fermo = st.date_input("Data fermo")
        turno_fermo = st.selectbox("Turno fermo", ["1", "2", "3"])
        causale_fermo = st.selectbox("Causale fermo", CAUSALI_FERMO)

        c1, c2 = st.columns(2)
        with c1:
            ora_inizio_fermo = st.time_input("Ora inizio fermo")
        with c2:
            ora_fine_fermo = st.time_input("Ora fine fermo")

        durata_h = ore_da_orari(ora_inizio_fermo, ora_fine_fermo)
        st.metric("Durata fermo calcolata", f"{durata_h:.2f} h")

        note_fermo = st.text_area("Note fermo")
        salva_fermo = st.form_submit_button("Salva fermo")

    if salva_fermo:
        if durata_h <= 0:
            st.error("Controlla ora inizio e ora fine fermo.")
        else:
            df = leggi_fermi()
            nuova = pd.DataFrame([{
                "id_fermo": datetime.now().strftime("%Y%m%d%H%M%S"),
                "data": data_fermo,
                "turno": turno_fermo,
                "ora_inizio": str(ora_inizio_fermo),
                "ora_fine": str(ora_fine_fermo),
                "durata_h": durata_h,
                "causale_fermo": causale_fermo,
                "note_fermo": note_fermo,
            }])
            df = pd.concat([df, nuova], ignore_index=True)
            df.to_csv(FERMI_FILE, index=False)
            st.session_state["salvato"] = True
            st.rerun()

with tab_qualita:
    st.subheader("Lotti in attesa analisi")

    df = prepara_produzioni(leggi_produzioni())
    attesa = df[df["stato_analisi"] == "ATTESA_ANALISI"].copy() if not df.empty else df

    if attesa.empty:
        st.info("Nessun lotto in attesa analisi.")
    else:
        attesa["label"] = (
            attesa["lotto"].astype(str) + " | " +
            attesa["codice_prodotto"].astype(str) + " | " +
            attesa["data"].dt.strftime("%d/%m/%Y")
        )

        lotto_label = st.selectbox("Seleziona lotto da rilasciare", attesa["label"].tolist())
        lotto_sel = attesa[attesa["label"] == lotto_label].iloc[0]

        st.write("Prodotto:", lotto_sel["descrizione_prodotto"])
        st.write("Kg prodotti:", lotto_sel["kg_polvere_reale"])

        with st.form("form_qualita"):
            esito = st.radio("Esito analisi", ["CONFORME", "NON_CONFORME"], horizontal=True)

            if esito == "CONFORME":
                kg_conformi = lotto_sel["kg_polvere_reale"]
                kg_nc = 0
                causa_nc = ""
                note_nc = ""
                st.metric("Kg conformi", f"{kg_conformi:.2f}")
            else:
                kg_conformi = st.number_input("Kg conformi", min_value=0.0, max_value=float(lotto_sel["kg_polvere_reale"]), step=1.0)
                kg_nc = max(float(lotto_sel["kg_polvere_reale"]) - kg_conformi, 0)
                st.metric("Kg NC", f"{kg_nc:.2f}")
                causa_nc = st.selectbox("Causa NC", CAUSE_NC)
                note_nc = st.text_area("Note NC")

            data_rilascio = st.date_input("Data rilascio analisi")
            salva_analisi = st.form_submit_button("Salva esito analisi")

        if salva_analisi:
            df_all = leggi_produzioni()
            mask = df_all["id_lotto"].astype(str) == str(lotto_sel["id_lotto"])
            df_all.loc[mask, "stato_analisi"] = esito
            df_all.loc[mask, "kg_conformi"] = kg_conformi
            df_all.loc[mask, "kg_nc"] = kg_nc
            df_all.loc[mask, "causa_nc"] = causa_nc
            df_all.loc[mask, "note_nc"] = note_nc
            df_all.loc[mask, "data_rilascio_analisi"] = data_rilascio
            df_all.to_csv(PRODUZIONI_FILE, index=False)
            st.session_state["salvato"] = True
            st.rerun()

with tab_cruscotto:
    st.subheader("Cruscotto OEE")

    prod = prepara_produzioni(leggi_produzioni())
    fermi = prepara_fermi(leggi_fermi())

    if prod.empty:
        st.info("Nessuna produzione registrata.")
    else:
        min_data = prod["data"].min().date()
        max_data = prod["data"].max().date()

        c1, c2, c3 = st.columns(3)
        with c1:
            periodo = st.date_input("Periodo", value=[min_data, max_data])
        with c2:
            prodotti_filtro = ["Tutti"] + sorted(prod["codice_prodotto"].dropna().unique().tolist())
            filtro_prodotto = st.selectbox("Filtro prodotto", prodotti_filtro)
        with c3:
            turni_filtro = ["Tutti"] + sorted(prod["turno"].dropna().astype(str).unique().tolist())
            filtro_turno = st.selectbox("Filtro turno", turni_filtro)

        prod_f = filtra_periodo(prod, "data", periodo)
        fermi_f = filtra_periodo(fermi, "data", periodo) if not fermi.empty else fermi

        if filtro_prodotto != "Tutti":
            prod_f = prod_f[prod_f["codice_prodotto"] == filtro_prodotto]

        if filtro_turno != "Tutti":
            prod_f = prod_f[prod_f["turno"].astype(str) == filtro_turno]
            if not fermi_f.empty:
                fermi_f = fermi_f[fermi_f["turno"].astype(str) == filtro_turno]

        kpi = calcola_cruscotto(prod_f, fermi_f)

        col_a, col_b = st.columns([1.2, 1])
        with col_a:
            st.plotly_chart(gauge_oee(kpi["oee"]), use_container_width=True)
        with col_b:
            st.metric("Disponibilità operativa", f"{kpi['disp_operativa']:.1%}")
            st.metric("Disponibilità tecnica", f"{kpi['disp_tecnica']:.1%}")
            st.metric("Performance", f"{kpi['performance']:.1%}")
            st.metric("Qualità", f"{kpi['quality']:.1%}")

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Kg molle", f"{kpi['kg_molle']:.1f}")
        m2.metric("Kg polvere teorica", f"{kpi['kg_teorici']:.1f}")
        m3.metric("Kg polvere reale", f"{kpi['kg_reali']:.1f}")
        m4.metric("Resa processo", f"{kpi['resa']:.1%}")

        q1, q2, q3, q4 = st.columns(4)
        q1.metric("Lotti prodotti", len(prod_f))
        q2.metric("Attesa analisi", len(prod_f[prod_f["stato_analisi"] == "ATTESA_ANALISI"]))
        q3.metric("Lotti conformi", len(prod_f[prod_f["stato_analisi"] == "CONFORME"]))
        q4.metric("Lotti NC", len(prod_f[prod_f["stato_analisi"] == "NON_CONFORME"]))

        st.divider()

        c_left, c_right = st.columns(2)

        with c_left:
            if fermi_f.empty:
                st.info("Nessun fermo nel periodo.")
            else:
                fermi_causa = fermi_f.groupby("causale_fermo", as_index=False)["durata_h"].sum()
                st.plotly_chart(
                    px.bar(fermi_causa, x="causale_fermo", y="durata_h", title="Ore fermo per causale"),
                    use_container_width=True
                )

        with c_right:
            nc = prod_f[prod_f["stato_analisi"] == "NON_CONFORME"].copy()
            if nc.empty:
                st.info("Nessuna NC nel periodo.")
            else:
                nc_causa = nc.groupby("causa_nc", as_index=False)["kg_nc"].sum()
                st.plotly_chart(
                    px.bar(nc_causa, x="causa_nc", y="kg_nc", title="Kg NC per causa"),
                    use_container_width=True
                )

        st.subheader("Produzioni filtrate")
        st.dataframe(prod_f, use_container_width=True)

with tab_formule:
    st.subheader("Assunzioni e formule")

    st.markdown("""
### 1. Turnazione

La dashboard assume **8 ore pianificate per turno**.

`tempo pianificato = numero turni con produzione × 8`

---

### 2. Polvere teorica

`kg polvere teorica = kg molle × residuo secco % / 100`

---

### 3. Resa processo

`resa = kg polvere reale / kg polvere teorica`

---

### 4. Disponibilità operativa

Tiene conto di tutti i fermi registrati.

`disponibilità operativa = (tempo pianificato - tutti i fermi) / tempo pianificato`

---

### 5. Disponibilità tecnica

Tiene conto solo dei fermi tecnici:

- Guasto
- Manutenzione straordinaria

`disponibilità tecnica = (tempo pianificato - fermi tecnici) / tempo pianificato`

---

### 6. Performance

La performance usa la capacità nominale presente nell'anagrafica prodotto.

`performance = kg polvere reale / (capacità kg/h prodotto × ore produzione)`

Se non viene impostata una capacità specifica, viene usato il default **20 kg/h**.

---

### 7. Qualità

La qualità viene calcolata solo sui lotti già rilasciati dal laboratorio.

`qualità = kg conformi / kg prodotti analizzati`

I lotti in **ATTESA_ANALISI** non entrano nel calcolo della qualità.

---

### 8. OEE

`OEE = disponibilità operativa × performance × qualità`

La disponibilità tecnica viene mostrata come KPI separato per distinguere i problemi macchina dai problemi organizzativi.
""")

with tab_prodotti:
    st.subheader("Anagrafica prodotti")

    prodotti = leggi_prodotti()
    st.dataframe(prodotti, use_container_width=True)

    with st.form("form_prodotto"):
        st.markdown("### Aggiungi o aggiorna prodotto")
        codice = st.text_input("Codice prodotto")
        descrizione = st.text_input("Descrizione prodotto")
        capacita = st.number_input("Capacità nominale kg/h", min_value=0.0, value=20.0, step=1.0)
        salva_prod = st.form_submit_button("Salva prodotto")

    if salva_prod:
        if codice.strip() == "" or descrizione.strip() == "":
            st.error("Inserisci codice e descrizione.")
        else:
            nuovo = pd.DataFrame([{
                "codice_prodotto": codice.strip(),
                "descrizione_prodotto": descrizione.strip(),
                "capacita_kg_h": capacita,
            }])
            prodotti = pd.concat([prodotti, nuovo], ignore_index=True)
            prodotti = prodotti.drop_duplicates(subset=["codice_prodotto"], keep="last")
            prodotti.to_csv(PRODOTTI_FILE, index=False)
            st.session_state["salvato"] = True
            st.rerun()

with tab_gestione:
    st.subheader("Gestione dati")

    st.markdown("### Produzioni")
    prod = leggi_produzioni()
    st.dataframe(prod, use_container_width=True)

    if not prod.empty:
        id_lotto = st.selectbox("Produzione da eliminare", prod["id_lotto"].astype(str).tolist())
        if st.button("Elimina produzione selezionata"):
            prod = prod[prod["id_lotto"].astype(str) != str(id_lotto)]
            prod.to_csv(PRODUZIONI_FILE, index=False)
            st.session_state["salvato"] = True
            st.rerun()

    st.markdown("### Fermi")
    fermi = leggi_fermi()
    st.dataframe(fermi, use_container_width=True)

    if not fermi.empty:
        id_fermo = st.selectbox("Fermo da eliminare", fermi["id_fermo"].astype(str).tolist())
        if st.button("Elimina fermo selezionato"):
            fermi = fermi[fermi["id_fermo"].astype(str) != str(id_fermo)]
            fermi.to_csv(FERMI_FILE, index=False)
            st.session_state["salvato"] = True
            st.rerun()

    st.divider()

    if st.button("Cancella tutto lo storico produzioni"):
        pd.DataFrame(columns=COL_PRODUZIONI).to_csv(PRODUZIONI_FILE, index=False)
        st.session_state["salvato"] = True
        st.rerun()

    if st.button("Cancella tutto lo storico fermi"):
        pd.DataFrame(columns=COL_FERMI).to_csv(FERMI_FILE, index=False)
        st.session_state["salvato"] = True
        st.rerun()
