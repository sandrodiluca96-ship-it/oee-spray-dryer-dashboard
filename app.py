...
/* Pulsanti secondari: bordo più evidente */
...
with tab_turno:
    ...
    indice_modifica = st.session_state.get("evento_da_modificare")
    evento_modifica = None

    if indice_modifica is not None and indice_modifica < len(st.session_state["eventi_turno"]):
        evento_modifica = st.session_state["eventi_turno"][indice_modifica]
        st.info(f"Modalità modifica attiva: evento {indice_modifica + 1} - {evento_modifica['tipo_evento']}")
    else:
        st.session_state["evento_da_modificare"] = None

    default_tipo = evento_modifica["tipo_evento"] if evento_modifica else "Produzione"
    default_tipo_index = EVENTI.index(default_tipo) if default_tipo in EVENTI else 0
    ...
    with st.form("form_evento", clear_on_submit=(evento_modifica is None)):
        tipo_evento = st.selectbox("Tipo evento", EVENTI, index=default_tipo_index)

        default_start = parse_time_value(evento_modifica.get("ora_inizio") if evento_modifica else None, TURNI[turno]["inizio"])
        default_end = parse_time_value(evento_modifica.get("ora_fine") if evento_modifica else None, TURNI[turno]["fine"])
        ...
        if tipo_evento == "Produzione":
            default_codice = evento_modifica.get("codice_prodotto") if evento_modifica else None
            labels = prodotti["label"].tolist()
            default_prodotto_index = 0

            if default_codice:
                match = prodotti.index[prodotti["codice_prodotto"].astype(str) == str(default_codice)].tolist()
                if match:
                    default_prodotto_index = prodotti.index.get_loc(match[0])

            prodotto_label = st.selectbox("Prodotto", labels, index=default_prodotto_index)
            prodotto = prodotti[prodotti["label"] == prodotto_label].iloc[0]

            lotto = st.text_input("Lotto", value=evento_modifica.get("lotto", "") if evento_modifica else "")
            ...
            with p1:
                kg_molle = st.number_input(
                    ...
                    min_value=0.0,
                    step=1.0,
                    value=float(evento_modifica.get("kg_molle", 0)) if evento_modifica else 0.0
                )
            with p2:
                residuo_secco = st.number_input(
                    ...
                    max_value=100.0,
                    step=0.1,
                    value=float(evento_modifica.get("residuo_secco", 0)) if evento_modifica else 0.0
                )

            kg_teorici = kg_molle * residuo_secco / 100
            st.metric("Kg polvere teorica", f"{kg_teorici:.2f} kg")

            kg_polvere = st.number_input(
                "Kg polvere prodotta",
                min_value=0.0,
                step=1.0,
                value=float(evento_modifica.get("kg_polvere_reale", 0)) if evento_modifica else 0.0
            )
            ...
            note = st.text_area("Note evento", value=evento_modifica.get("note", "") if evento_modifica else "")
            evento.update({
                "codice_prodotto": "",
        ...
        if evento_modifica is None:
            aggiungi = st.form_submit_button("Aggiungi evento al turno", type="primary")
            salva_modifiche = False
            annulla_modifica = False
        else:
            c_mod1, c_mod2 = st.columns(2)
            with c_mod1:
                salva_modifiche = st.form_submit_button("Salva modifiche evento", type="primary")
            with c_mod2:
                annulla_modifica = st.form_submit_button("Annulla modifica", type="secondary")
            aggiungi = False

    if aggiungi or salva_modifiche:
        if durata_h <= 0:
            ...
            if salva_modifiche and indice_modifica is not None:
                st.session_state["eventi_turno"][indice_modifica] = evento
                st.session_state["evento_da_modificare"] = None
                st.success("Evento modificato correttamente.")
            else:
    ...
    if eventi_df.empty:
        ...
        with ge3:
            if st.button("Duplica evento", type="secondary"):
                nuovo_evento = st.session_state["eventi_turno"][indice_evento].copy()
                st.session_state["eventi_turno"].insert(indice_evento + 1, nuovo_evento)
                st.rerun()

        evento_sel = eventi_df.iloc[indice_evento]
        st.info(
            f"Evento selezionato: {indice_evento + 1} - {evento_sel['tipo_evento']} | "
            f"{evento_sel['ora_inizio']} - {evento_sel['ora_fine']}"
        )
        ...
        scostamento_turno = copertura_ore - TURNO_H

        if abs(scostamento_turno) <= 0.01:
            st.markdown(
                "<div class='alert-ok'>Copertura turno corretta: gli eventi coprono le 8 ore pianificate.</div>",
                unsafe_allow_html=True
            )
        elif scostamento_turno > 0:
            st.markdown(
                f"<div class='alert-standby'>Attenzione: gli eventi superano il turno di {scostamento_turno:.2f} h. Il turno resta in stand-by finché non viene corretto.</div>",
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                f"<div class='alert-standby'>Attenzione: mancano {abs(scostamento_turno):.2f} h per coprire le 8 ore del turno. Il turno resta in stand-by finché non viene completato.</div>",
                unsafe_allow_html=True
            )
            ...
            if st.button("Salva turno", type="primary"):
                ...
                if abs(copertura_ore - TURNO_H) > 0.01:
                    st.error(
                        ...
                        "I dati restano in stand-by nella schermata: correggi o aggiungi gli eventi mancanti."
                    )
                    st.stop()
...
with tab_qualita:
    ...
    if attesa.empty:
        ...
        with st.form("form_qualita"):
            ...
            salva_esito = st.form_submit_button("Salva esito", type="primary")
...
with tab_gestione:
    ...
    if not prod.empty:
        id_lotto = st.selectbox(
            "Seleziona produzione da modificare/eliminare",
            prod["id_lotto"].astype(str).tolist(),
            key="gestione_id_lotto"
        )

        prod_sel = prod[prod["id_lotto"].astype(str) == str(id_lotto)].iloc[0]

        with st.form("form_modifica_produzione"):
            st.markdown("### Modifica produzione")
            ...
            with c4:
                nuovo_kg_molle = st.number_input(
                    "Kg molle",
                    min_value=0.0,
                    value=safe_float(prod_sel["kg_molle"]),
                    step=1.0
                )
            with c5:
                nuovo_rs = st.number_input(
                    "Residuo secco %",
                    min_value=0.0,
                    max_value=100.0,
                    value=safe_float(prod_sel["residuo_secco"]),
                    step=0.1
                )
            with c6:
                nuovo_kg_reale = st.number_input(
                    "Kg polvere reale",
                    min_value=0.0,
                    value=safe_float(prod_sel["kg_polvere_reale"]),
                    step=1.0
                )
            ...
            with c9:
                nuove_ore_produzione = st.number_input(
                    "Ore produzione",
                    min_value=0.0,
                    value=safe_float(prod_sel["ore_produzione"]),
                    step=0.25
                )

            stato_corrente = str(prod_sel["stato_analisi"]) if str(prod_sel["stato_analisi"]) in ["ATTESA_ANALISI", "CONFORME", "NON_CONFORME"] else "ATTESA_ANALISI"
            nuovo_stato = st.selectbox(
                "Stato analisi",
                ["ATTESA_ANALISI", "CONFORME", "NON_CONFORME"],
                index=["ATTESA_ANALISI", "CONFORME", "NON_CONFORME"].index(stato_corrente)
            )

            c10, c11 = st.columns(2)
            with c10:
                nuovo_kg_conformi = st.number_input(
                    "Kg conformi",
                    min_value=0.0,
                    value=safe_float(prod_sel["kg_conformi"]),
                    step=1.0
                )
            with c11:
                nuovo_kg_nc = st.number_input(
                    "Kg NC",
                    min_value=0.0,
                    value=safe_float(prod_sel["kg_nc"]),
                    step=1.0
                )

            nuova_causa_nc = st.text_input("Causa NC", value="" if pd.isna(prod_sel["causa_nc"]) else str(prod_sel["causa_nc"]))
            nuove_note_nc = st.text_area("Note NC", value="" if pd.isna(prod_sel["note_nc"]) else str(prod_sel["note_nc"]))
        ...
        if salva_mod_prod:
            ...
            prod.loc[mask, "data"] = str(nuova_data)
            prod.loc[mask, "turno"] = nuovo_turno
            prod.loc[mask, "lotto"] = nuovo_lotto
            prod.loc[mask, "codice_prodotto"] = nuovo_codice
            prod.loc[mask, "descrizione_prodotto"] = nuova_descrizione
            prod.loc[mask, "kg_molle"] = nuovo_kg_molle
            prod.loc[mask, "residuo_secco"] = nuovo_rs
            prod.loc[mask, "kg_polvere_teorica"] = nuovo_kg_teorico
            prod.loc[mask, "kg_polvere_reale"] = nuovo_kg_reale
            prod.loc[mask, "ora_inizio"] = nuova_ora_inizio
            prod.loc[mask, "ora_fine"] = nuova_ora_fine
            prod.loc[mask, "ore_produzione"] = nuove_ore_produzione
            prod.loc[mask, "stato_analisi"] = nuovo_stato
            prod.loc[mask, "kg_conformi"] = nuovo_kg_conformi
            prod.loc[mask, "kg_nc"] = nuovo_kg_nc
            prod.loc[mask, "causa_nc"] = nuova_causa_nc
            prod.loc[mask, "note_nc"] = nuove_note_nc
    ...
    if not fermi.empty:
        id_fermo = st.selectbox(
            "Seleziona fermo da modificare/eliminare",
            fermi["id_fermo"].astype(str).tolist(),
            key="gestione_id_fermo"
        )

        fermo_sel = fermi[fermi["id_fermo"].astype(str) == str(id_fermo)].iloc[0]

        with st.form("form_modifica_fermo"):
            st.markdown("### Modifica fermo")

            c1, c2, c3 = st.columns(3)
            with c1:
                nuova_data_fermo = st.date_input(
                    "Data fermo",
            ...
            with c2:
                nuovo_turno_fermo = st.selectbox(
                    "Turno fermo",
                    ["1", "2", "3"],
            ...
            with c3:
                ...
                nuova_causale = st.selectbox(
                    "Causale fermo",
                    EVENTI[1:],
                    index=EVENTI[1:].index(causale_corrente) if causale_corrente in EVENTI[1:] else 0
                )

            c4, c5, c6 = st.columns(3)
            with c4:
                nuova_ora_inizio_fermo = st.text_input("Ora inizio fermo", value=str(fermo_sel["ora_inizio"]))
            with c5:
                nuova_ora_fine_fermo = st.text_input("Ora fine fermo", value=str(fermo_sel["ora_fine"]))
            with c6:
                nuova_durata_fermo = st.number_input(
                    "Durata fermo h",
                    min_value=0.0,
                    value=safe_float(fermo_sel["durata_h"]),
                    step=0.25
                )

            nuove_note_fermo = st.text_area(
                "Note fermo",
                value="" if pd.isna(fermo_sel["note_fermo"]) else str(fermo_sel["note_fermo"])
            )
        ...
        if salva_mod_fermo:
            ...
            fermi.loc[mask, "data"] = str(nuova_data_fermo)
            fermi.loc[mask, "turno"] = nuovo_turno_fermo
            fermi.loc[mask, "causale_fermo"] = nuova_causale
            fermi.loc[mask, "ora_inizio"] = nuova_ora_inizio_fermo
            fermi.loc[mask, "ora_fine"] = nuova_ora_fine_fermo
            fermi.loc[mask, "durata_h"] = nuova_durata_fermo
            fermi.loc[mask, "note_fermo"] = nuove_note_fermo
