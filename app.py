        ...
        if attesa.empty:
            ...
            if salva_esito:
                ...
                if not mask.any():
                    ...
                    df.loc[mask, "data_rilascio_analisi"] = str(data_rilascio)
...
with tab_gestione:
    ...
    if not prod.empty:
        ...
        if salva_mod_prod:
            ...
            prod.loc[mask, "ore_produzione"] = nuove_ore
            prod.loc[mask, "stato_analisi"] = nuovo_stato
            prod.loc[mask, "kg_conformi"] = nuovo_kg_conf
            prod.loc[mask, "kg_nc"] = nuovo_kg_nc
            prod.loc[mask, "causa_nc"] = nuova_causa
            prod.loc[mask, "note_nc"] = nuove_note
    ...
    if not fermi.empty:
        ...
        if salva_mod_fermo:
            ...
            fermi.loc[mask, "data"] = str(nuova_data_f)
            fermi.loc[mask, "turno"] = nuovo_turno_f
            fermi.loc[mask, "causale_fermo"] = nuova_causale
            fermi.loc[mask, "ora_inizio"] = nuova_ora_i_f
            fermi.loc[mask, "ora_fine"] = nuova_ora_f_f
            fermi.loc[mask, "durata_h"] = nuova_durata
            fermi.loc[mask, "note_fermo"] = nuove_note_f
