...
# Release operativa 5.8.0: qualità OEE standard fissa al 95% e KPI Production vs Target aggiornati.
...
def leggi_storico_mensile():
    ...
    if not df.empty:
        df["kg_semilavorato_totale"] = pd.to_numeric(df["kg_semilavorato_totale"], errors="coerce").fillna(0)
        df["kg_puro_ottenuto"] = pd.to_numeric(df["kg_puro_ottenuto"], errors="coerce").fillna(0)
        df["kg_puro_equivalente"] = pd.to_numeric(df["kg_puro_equivalente"], errors="coerce").fillna(0)
...
def minuti_assoluti_turno(t, turno_corrente):
    """Converte un orario in minuti relativi al turno selezionato.
    Serve a validare correttamente anche il turno notturno 22:00-06:00.
    """
    if t is None or turno_corrente not in TURNI:
        return None

    inizio = TURNI[turno_corrente]["inizio"]
    start_min = inizio.hour * 60 + inizio.minute
...
def evento_dentro_turno(ora_inizio, ora_fine, turno_corrente):
    """Verifica che l'evento sia completamente interno alla finestra del turno selezionato."""
    if ora_inizio is None or ora_fine is None or turno_corrente not in TURNI:
        return False

    inizio = TURNI[turno_corrente]["inizio"]
    fine = TURNI[turno_corrente]["fine"]

    start_min = inizio.hour * 60 + inizio.minute
    end_min = fine.hour * 60 + fine.minute
    if end_min <= start_min:
        end_min += 24 * 60

    ev_start = minuti_assoluti_turno(ora_inizio, turno_corrente)
    ev_end = minuti_assoluti_turno(ora_fine, turno_corrente)

    if ev_end <= ev_start:
        ev_end += 24 * 60

    return ev_start >= start_min and ev_end <= end_min


def timestamp_evento(data_turno, turno_corrente, ora_inizio, ora_fine):
    """Restituisce i timestamp reali mantenendo separata la data operativa del turno.

    Per il turno 3, gli orari 00:00-06:00 ricadono nel giorno successivo alla
    data selezionata. La data del turno resta invece quella di inizio turno.
    """
    if data_turno is None or turno_corrente not in TURNI or ora_inizio is None or ora_fine is None:
        return None, None

    data_inizio = data_turno
    if turno_corrente == "3" and ora_inizio < TURNI[turno_corrente]["inizio"]:
        data_inizio = data_turno + timedelta(days=1)

    data_fine = data_turno
    if turno_corrente == "3" and ora_fine <= TURNI[turno_corrente]["fine"]:
        data_fine = data_turno + timedelta(days=1)

    dt_inizio = datetime.combine(data_inizio, ora_inizio)
    dt_fine = datetime.combine(data_fine, ora_fine)
...
def valida_timeline_turno(eventi_buffer, turno_corrente, richiedi_completo=False):
    """Controlla durata, sovrapposizioni, vuoti e copertura del singolo turno."""
    if turno_corrente not in TURNI:
        return False, "Seleziona il turno."
    if not eventi_buffer:
        return False, "Inserisci almeno un evento."

    intervalli = []
    for i, ev in enumerate(eventi_buffer, start=1):
        oi = parse_time_str(ev.get("ora_inizio"), None)
        of = parse_time_str(ev.get("ora_fine"), None)
        if oi is None or of is None or not evento_dentro_turno(oi, of, turno_corrente):
            return False, f"L'evento {i} non rientra nel turno selezionato."
        start = minuti_assoluti_turno(oi, turno_corrente)
        end = minuti_assoluti_turno(of, turno_corrente)
        if end <= start:
            end += 24 * 60
        intervalli.append((start, end, i))

    intervalli.sort(key=lambda x: x[0])
    turno_start = TURNI[turno_corrente]["inizio"]
    turno_end = TURNI[turno_corrente]["fine"]
    start_atteso = turno_start.hour * 60 + turno_start.minute
    end_atteso = turno_end.hour * 60 + turno_end.minute
    if end_atteso <= start_atteso:
        end_atteso += 24 * 60

    precedente = start_atteso
    for start, end, idx in intervalli:
        if start < precedente:
            return False, f"L'evento {idx} si sovrappone all'evento precedente."
        if start > precedente and richiedi_completo:
            return False, f"Intervallo non coperto tra {precedente % 1440 // 60:02d}:{precedente % 60:02d} e {start % 1440 // 60:02d}:{start % 60:02d}."
        precedente = end

    if precedente > end_atteso:
        return False, "Gli eventi superano la fine del turno."
    if richiedi_completo and precedente < end_atteso:
        return False, f"Turno incompleto: gli eventi terminano alle {precedente % 1440 // 60:02d}:{precedente % 60:02d}."

    return True, "Timeline valida."


def ultimo_orario_fine_buffer():
    buffer = pd.DataFrame(st.session_state.get("eventi_turno_buffer", []))
    if buffer.empty or "ora_fine" not in buffer.columns:
        return None
    val = str(buffer.iloc[-1].get("ora_fine", ""))[:5]
    return val if val and val != "nan" else None


def turno_completo():
    buffer = pd.DataFrame(st.session_state.get("eventi_turno_buffer", []))
    if buffer.empty or "durata_h" not in buffer.columns:
        return False
    durata = pd.to_numeric(buffer["durata_h"], errors="coerce").fillna(0).sum()
    return durata >= TURNO_H - 0.01
...
def aggiorna_lotti_da_eventi():
    ...
    for lotto, grp in prod_events.groupby("lotto_molle"):
        ...
        if not existing.empty and str(existing.iloc[0].get("stato_analisi", "")) in ["CONFORME", "NON_CONFORME"]:
            esito_esistente = str(existing.iloc[0]["stato_analisi"])
            kg_conformi_esistenti = safe_float(existing.iloc[0].get("kg_conformi", 0))
            kg_nc_esistenti = safe_float(existing.iloc[0].get("kg_nc", 0))

            if esito_esistente == "CONFORME":
                # Un lotto conforme è conforme per l'intera quantità finale.
                # Se i kg finali vengono rettificati, anche i kg conformi devono
                # essere riallineati per impedire una qualità superiore al 100%.
                stato_analisi = "CONFORME"
                kg_conformi = kg_finale
                kg_nc = 0
                causa_nc = ""
                note_nc = existing.iloc[0].get("note_nc", "")
                data_rilascio = existing.iloc[0].get("data_rilascio_analisi", "")
            elif abs((kg_conformi_esistenti + kg_nc_esistenti) - kg_finale) <= 0.01:
                # Mantiene un esito NC solo se le quantità qualità continuano a
                # riconciliare esattamente con la nuova quantità prodotta.
                stato_analisi = "NON_CONFORME"
                kg_conformi = kg_conformi_esistenti
                kg_nc = kg_nc_esistenti
                causa_nc = existing.iloc[0].get("causa_nc", "")
                note_nc = existing.iloc[0].get("note_nc", "")
                data_rilascio = existing.iloc[0].get("data_rilascio_analisi", "")
            else:
                # Una rettifica della quantità rende non più valido il precedente
                # riparto conforme/NC: il lotto torna in attesa di rilascio.
                stato_analisi = "ATTESA_ANALISI"
                kg_conformi = 0
                kg_nc = 0
                causa_nc = ""
                note_nc = "Quantità finale rettificata: aggiornare l'esito qualità."
                data_rilascio = ""
        else:
            kg_conformi = 0
            kg_nc = 0
        ...
        # Impostazione gestionale: i lotti chiusi sono considerati conformi
        # automaticamente. Il fattore Qualità applicato all'OEE resta fisso al
        # 95% e non richiede un rilascio manuale del laboratorio.
        if stato_lotto == "CHIUSO":
            stato_analisi = "CONFORME"
            kg_conformi = kg_finale
            kg_nc = 0
            causa_nc = ""
            note_nc = ""
            if not data_rilascio:
                data_rilascio = data_chiusura
...
def calcola_kpi(eventi, lotti):
    ...
    # Estratto puro realmente contenuto nella polvere finale. Il precedente
    # valore (kg_secco_puro) rappresentava invece il secco teorico in ingresso.
    kg_puro = (
        (
            lotti_chiusi["kg_polvere_finale"]
            * lotti_chiusi["estratto_puro_pct"]
            / 100
        ).sum()
    ...
    # Assunzione gestionale temporanea: qualità standard fissa al 95% per tutti
    # i lotti chiusi, senza attesa o conferma manuale dell'analisi.
    quality = QUALITA_STANDARD if not lotti_chiusi.empty else 0
...
def prepara_lotti_per_production_report(lotti):
    ...
    out["kg_puro_ottenuto"] = out["kg_polvere_finale"] * out["estratto_puro_pct"] / 100
    return out
...
def somma_periodo(lotti, inizio=None, fine=None):
    if lotti.empty:
        return {
            "kg_semilavorato_totale": 0.0,
            "kg_puro_ottenuto": 0.0,
            "kg_puro_equivalente": 0.0,
            "lotti": 0,
        }
    df = lotti.copy()
    ...
    return {
        "kg_semilavorato_totale": float(df["kg_polvere_finale"].sum()) if not df.empty else 0.0,
        "kg_puro_ottenuto": float(df["kg_puro_ottenuto"].sum()) if not df.empty else 0.0,
        "kg_puro_equivalente": float(df["kg_equivalenti_standard"].sum()) if not df.empty else 0.0,
        "lotti": int(len(df)),
    }
...
if "next_event_start" not in st.session_state:
    st.session_state["next_event_start"] = None

if "event_form_reset_counter" not in st.session_state:
    st.session_state["event_form_reset_counter"] = 0

if "buffer_data_turno" not in st.session_state:
    st.session_state["buffer_data_turno"] = None

if "buffer_turno" not in st.session_state:
    st.session_state["buffer_turno"] = None
...
def reset_buffer(reset_header=False):
    ...
    if reset_header:
        ...
        st.session_state["next_event_start"] = None
        st.session_state["event_form_reset_counter"] = st.session_state.get("event_form_reset_counter", 0) + 1
...
st.caption("Versione 5.8.0 – Qualità standard 95% | Production vs Target aggiornato")
...
if sezione_corrente == "Turno":
    ...
    buffer_presente = len(st.session_state["eventi_turno_buffer"]) > 0
    c0, c1 = st.columns(2)
    with c0:
        data_turno = st.date_input("Data turno", value=None, format="DD/MM/YYYY", key=f"header_data_turno_{header_key_suffix}", disabled=buffer_presente)
    with c1:
        turno_sel = st.selectbox("Turno", ["-- Seleziona turno --", "1", "2", "3"], index=0, key=f"header_turno_{header_key_suffix}", disabled=buffer_presente)
        turno = None if turno_sel == "-- Seleziona turno --" else turno_sel

    if buffer_presente:
        st.caption("Data e turno sono bloccati fino al salvataggio o allo svuotamento degli eventi.")
    ...
    if turno_completo():
        st.markdown(
            "<div class='alert-ok'>Turno già coperto: gli eventi raggiungono le 8 ore pianificate. "
            "Per aggiungere altro, modifica o elimina un evento già inserito.</div>",
            unsafe_allow_html=True,
        )
        aggiunta_evento_bloccata = True
    else:
        aggiunta_evento_bloccata = False
    ...
    tipo_evento = st.selectbox("Tipo evento", tipi_evento_disponibili, index=None if evento_mod is None else default_tipo_index, placeholder="Seleziona tipo evento", key=f"tipo_evento_select_{st.session_state.get('event_form_reset_counter', 0)}")
    ...
    with st.container():
        # Mostra solo gli orari appartenenti al turno selezionato.
        # Mantiene incluso l'orario di fine turno per consentire la chiusura dell'ultimo evento.
        def lista_orari_turno_15min(turno_corrente):
            if turno_corrente not in TURNI:
                return lista_orari_15min()

            inizio = TURNI[turno_corrente]["inizio"]
            fine = TURNI[turno_corrente]["fine"]

            minuti_inizio = inizio.hour * 60 + inizio.minute
            minuti_fine = fine.hour * 60 + fine.minute

            if minuti_fine <= minuti_inizio:
                minuti_fine += 24 * 60

            valori = []
            minuto = minuti_inizio
            while minuto <= minuti_fine:
                m = minuto % (24 * 60)
                valori.append(f"{m // 60:02d}:{m % 60:02d}")
                minuto += 15

            return valori

        orari = lista_orari_turno_15min(turno)
        if evento_mod:
            default_start_txt = str(evento_mod["ora_inizio"])[:5]
            default_end_txt = str(evento_mod["ora_fine"])[:5]
        else:
            default_start_txt = st.session_state.get("next_event_start") or ultimo_orario_fine_buffer()
            if not default_start_txt and turno in TURNI:
                default_start_txt = TURNI[turno]["inizio"].strftime("%H:%M")
            default_end_txt = None
        ...
        if evento_mod is None:
            submit_evento = st.button(
                "Aggiungi evento al turno",
                type="primary",
                key="btn_aggiungi_evento",
                disabled=aggiunta_evento_bloccata,
            )
    ...
    if submit_evento or submit_modifica:
        ...
        elif turno is not None and not evento_dentro_turno(ora_inizio, ora_fine, turno):
            st.error("Evento non valido: l'intervallo orario deve rientrare completamente nel turno selezionato.")
        elif tipo_evento == "Produzione" and durata_h < (1 / 60):
            st.error("La produzione deve durare almeno 1 minuto.")
        else:
            if not submit_modifica and not st.session_state["eventi_turno_buffer"]:
                st.session_state["buffer_data_turno"] = str(data_turno)
                st.session_state["buffer_turno"] = turno

            buffer_prova = list(st.session_state["eventi_turno_buffer"])
            if submit_modifica and idx_mod is not None:
                buffer_prova[idx_mod] = evento
            else:
                buffer_prova.append(evento)

            durata_prova = sum(safe_float(ev.get("durata_h", 0)) for ev in buffer_prova)
            timeline_ok, timeline_msg = valida_timeline_turno(buffer_prova, turno, richiedi_completo=False)

            if durata_prova > TURNO_H + 0.01:
                st.error(f"Evento non aggiunto: il turno supererebbe le {TURNO_H} ore pianificate.")
                st.stop()
            if not timeline_ok:
                st.error(f"Evento non aggiunto: {timeline_msg}")
                st.stop()

            st.session_state["eventi_turno_buffer"] = buffer_prova
            if submit_modifica:
                st.session_state["evento_da_modificare"] = None

            # Prepara il prossimo evento: tipo evento vuoto, campi puliti,
            # ora inizio proposta uguale all'ora fine dell'evento appena inserito.
            if not submit_modifica:
                st.session_state["next_event_start"] = ora_fine.strftime("%H:%M") if ora_fine else None
                st.session_state["event_form_reset_counter"] = st.session_state.get("event_form_reset_counter", 0) + 1
    ...
    if eventi_buffer.empty:
        ...
        with r2:
            if st.button("Salva turno", type="primary"):
                ...
                timeline_ok, timeline_msg = valida_timeline_turno(
                    st.session_state["eventi_turno_buffer"], turno, richiedi_completo=True
                )
                if not timeline_ok:
                    st.error("Turno non salvato: " + timeline_msg)
                    st.stop()
                ...
                for i, ev in enumerate(st.session_state["eventi_turno_buffer"], start=1):
                    oi = parse_time_str(ev.get("ora_inizio"), None)
                    of = parse_time_str(ev.get("ora_fine"), None)
                    dt_i, dt_f = timestamp_evento(data_turno, turno, oi, of)
                    row = {
                        "id_evento": f"{id_turno}-E{i}",
                        "id_turno": id_turno,
                        "data": str(data_turno),
                        "turno": turno,
                        "data_ora_inizio": dt_i.isoformat(sep=" ") if dt_i else "",
                        "data_ora_fine": dt_f.isoformat(sep=" ") if dt_f else "",
                    }
                    row.update(ev)
                    rows.append(row)
...
if sezione_corrente == "Production vs Target":
    ...
    st.caption("Daily, Weekly, Monthly, Year-to-Date | Semilavorato totale, puro ottenuto e puro equivalente")
    ...
    basis = st.radio(
        "Base di lettura",
        ["Semilavorato totale (kg)", "Puro ottenuto (kg)", "Puro equivalente (kg eq.)"],
        horizontal=True,
        help=(
            "Semilavorato totale usa i kg fisici finali; Puro ottenuto calcola i kg finali per la percentuale "
            "di estratto puro; Puro equivalente normalizza la produzione allo standard aziendale del 40%."
        ),
    )
    configurazione_basi = {
        "Semilavorato totale (kg)": {
            "actual": "kg_semilavorato_totale",
            "target": "kg_semilavorato_totale",
            "trend": "semilavorato_totale_kg",
        },
        "Puro ottenuto (kg)": {
            "actual": "kg_puro_ottenuto",
            "target": "kg_puro_ottenuto",
            "trend": "puro_ottenuto_kg",
        },
        "Puro equivalente (kg eq.)": {
            "actual": "kg_puro_equivalente",
            "target": "kg_puro_equivalente",
            "trend": "puro_equivalente_kg",
        },
    }
    cfg_basis = configurazione_basi[basis]
    col_target = cfg_basis["target"]
    label_valore = basis
    ...
    for nome, start, end, target in periods:
        ...
        actual = s_periodo[cfg_basis["actual"]]
        rows.append({
            "Period": nome,
            "Actual": actual,
    ...
    if lotti_report.empty:
        ...
        trend_g = trend.groupby("mese", as_index=False).agg(
            semilavorato_totale_kg=("kg_polvere_finale", "sum"),
            puro_ottenuto_kg=("kg_puro_ottenuto", "sum"),
            puro_equivalente_kg=("kg_equivalenti_standard", "sum"),
            lots=("lotto_molle", "count"),
        )
        y_col = cfg_basis["trend"]
    ...
    st.caption(
        "Formato atteso: mese, kg_semilavorato_totale, kg_puro_ottenuto, "
        "kg_puro_equivalente, giorni_produttivi, note. Esempio mese: 2025-01."
    )
...
if sezione_corrente == "Cruscotto OEE":
    ...
    m1.metric(
        "Semilavorato totale",
        f"{kpi['kg_fisici']:.1f} kg",
        help="Quantità fisica complessiva di polvere finale ottenuta dai lotti chiusi.",
    )
    m2.metric(
        "Puro ottenuto",
        f"{kpi['kg_puro']:.1f} kg",
        help="Estratto puro contenuto nel semilavorato finale: kg finali × percentuale di estratto puro.",
    )
    m3.metric(
        "Puro equivalente",
        f"{kpi['kg_equiv']:.1f} kg eq.",
        help="Produzione finale normalizzata allo standard aziendale con il 40% di estratto puro.",
    )
    m4.metric(
        "Taglio medio",
        f"{kpi['taglio_medio']:.1f}%",
        help="Percentuale media di maltodestrina sul semilavorato, ponderata sui kg finali prodotti.",
    )
...
La qualità è impostata temporaneamente come parametro gestionale fisso:

`Qualità standard = 95%`

I lotti chiusi sono considerati automaticamente conformi e non è richiesta una
conferma manuale dell'analisi ai fini del calcolo OEE.
...
if sezione_corrente == "Gestione dati":
    ...
    if not eventi.empty:
        ...
        with st.form("form_modifica_evento_salvato"):
            ...
            tipo_produzione_salvato = str(evento_sel.get("tipo_produzione", ""))
            modifica_chiusura = (
                str(evento_sel.get("tipo_evento", "")) == "Produzione"
                and tipo_produzione_salvato == "Chiusura lotto"
            )

            if modifica_chiusura:
                kg_finale_attuale = safe_float(evento_sel.get("kg_polvere_finale", 0))
                nuova_kg_polvere_finale = st.number_input(
                    "Kg polvere finale ottenuta",
                    min_value=0.0,
                    value=float(kg_finale_attuale),
                    step=1.0,
                    help="La modifica aggiorna automaticamente kg equivalenti, resa e KPI del lotto.",
                )
            else:
                nuova_kg_polvere_finale = safe_float(evento_sel.get("kg_polvere_finale", 0))
        ...
        if salva_modifica_evento:
            ...
            if nuova_durata <= 0:
            ...
            elif modifica_chiusura and nuova_kg_polvere_finale <= 0:
                st.error("Inserisci una quantità di polvere finale maggiore di zero.")
            else:
                mask = eventi["id_evento"].astype(str) == str(id_evento_mod)

                estratto_puro_pct = safe_float(evento_sel.get("estratto_puro_pct", 0))
                if modifica_chiusura and estratto_puro_pct <= 0:
                    dati_lotto = recupera_lotto_molle(nuovo_lotto)
                    if dati_lotto is not None:
                        estratto_puro_pct = safe_float(dati_lotto.get("estratto_puro_pct", 0))

                nuovi_kg_equivalenti = (
                    nuova_kg_polvere_finale
                    * safe_div(estratto_puro_pct / 100, STANDARD_ESTRATTO_PURO)
                    if modifica_chiusura else safe_float(evento_sel.get("kg_equivalenti_standard", 0))
                )
                ...
                if modifica_chiusura:
                    aggiornamenti.update({
                        "kg_polvere_finale": nuova_kg_polvere_finale,
                        "kg_equivalenti_standard": nuovi_kg_equivalenti,
                    })
