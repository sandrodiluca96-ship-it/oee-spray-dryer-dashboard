...
with tab_turno:
    ...
    c0, c1 = st.columns(2)
    with c0:
        data_turno = st.date_input("Data turno", value=None, format="YYYY/MM/DD")
    with c1:
        turno = st.selectbox("Turno", ["1", "2", "3"], index=None, placeholder="Seleziona turno")
    ...
    tipo_evento = st.selectbox(
        "Tipo evento",
        TIPI_EVENTO,
        index=default_tipo_index if evento_mod else None,
        placeholder="Seleziona tipo evento",
        key="tipo_evento_select"
    )

    if tipo_evento is None:
        st.caption("Seleziona il tipo evento per continuare.")
    ...
    with st.form("form_evento", clear_on_submit=False):
        default_start = parse_time_str(evento_mod["ora_inizio"] if evento_mod else "", TURNI[turno]["inizio"] if turno is not None else time(6, 0))
        default_end = parse_time_str(evento_mod["ora_fine"] if evento_mod else "", TURNI[turno]["fine"] if turno is not None else time(14, 0))
        ...
        evento = {
            "tipo_evento": "" if tipo_evento is None else tipo_evento,
            "tipo_produzione": "",
            "lotto_molle": "",
            "codice_prodotto": "",
            "descrizione_prodotto": "",
    ...
    if submit_evento or submit_modifica:
        if data_turno is None:
            st.error("Prima di aggiungere l'evento devi selezionare la data turno.")
            st.stop()

        if turno is None:
            st.error("Prima di aggiungere l'evento devi selezionare il turno.")
            st.stop()

        if tipo_evento is None:
            st.error("Devi selezionare il tipo evento.")
            st.stop()
    ...
    if eventi_buffer.empty:
        ...
        with r2:
            if st.button("Salva turno", type="primary"):
                if data_turno is None:
                    st.error("Turno non salvato: devi selezionare la data turno.")
                    st.stop()
...
with tab_cruscotto:
    ...
    st.caption("L’OEE viene mostrato sempre in modalità cruscotto/tachimetro.")
