...
with tab_turno:
    ...
    # Importante: il tipo evento è fuori dal form, così quando passi da fermo a Produzione
    # Streamlit aggiorna subito i campi prodotto/lotto/quantità.
    tipo_evento = st.selectbox("Tipo evento", TIPI_EVENTO, index=default_tipo_index, key="tipo_evento_corrente")

    if tipo_evento != "Produzione":
        st.caption("Evento di fermo: non sono richiesti prodotto, lotto, kg molle, residuo secco o kg polvere.")
    ...
    with st.form("form_evento", clear_on_submit=False):
        ...
        if tipo_evento == "Produzione":
            ...
            if prodotto_label == "-- Seleziona prodotto --":
                prodotto = None
            else:
    ...
    if submit_evento or submit_modifica:
        ...
        elif tipo_evento == "Produzione" and evento["codice_prodotto"].strip() == "":
            st.error("Per una produzione devi selezionare il prodotto.")
    ...
    if eventi_df.empty:
        ...
        with c_save:
            if st.button("Salva turno", type="primary"):
                ...
                # Il turno può essere salvato anche senza produzione.
                # In questo caso vengono salvati solo gli eventi di fermo e non viene generato nessun lotto in Qualità.
                id_turno = nuovo_id()
...
Gli eventi diversi da Produzione sono trattati come fermi e non richiedono prodotto, lotto o quantità.  
Un turno può essere salvato anche senza produzione, purché gli eventi coprano le 8 ore pianificate.
