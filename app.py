...
def reset_buffer(reset_header=False):
    ...
    # Se il turno non è ancora chiuso, data e turno restano valorizzati.
    # Dopo il salvataggio del turno completo, vengono azzerati.
    if reset_header:
        for key in [
            "data_turno",
            "turno",
...
if st.session_state["salvato"]:
    st.session_state["salvato"] = False
    st.success("Turno salvato correttamente. Il modulo è stato azzerato ed è pronto per un nuovo turno.")
...
with tab_turno:
    ...
    c0, c1 = st.columns(2)
    with c0:
        data_turno_txt = st.text_input("Data turno", value="", placeholder="gg/mm/aaaa")
        data_turno = parse_date_manual(data_turno_txt)
    with c1:
        turno_sel = st.selectbox("Turno", ["-- Seleziona turno --", "1", "2", "3"], index=0)
    ...
    tipo_evento = st.selectbox("Tipo evento", TIPI_EVENTO, index=None if evento_mod is None else default_tipo_index, placeholder="Seleziona tipo evento", key="tipo_evento_select")
    ...
    with st.form("form_evento", clear_on_submit=False):
        default_start_txt = str(evento_mod["ora_inizio"])[:5] if evento_mod else ""
        default_end_txt = str(evento_mod["ora_fine"])[:5] if evento_mod else ""

        c2, c3 = st.columns(2)
        with c2:
            ora_inizio_txt = st.text_input("Ora inizio evento", value=default_start_txt, placeholder="HH:MM")
        with c3:
            ora_fine_txt = st.text_input("Ora fine evento", value=default_end_txt, placeholder="HH:MM")

        ora_inizio = parse_time_manual(ora_inizio_txt)
        ora_fine = parse_time_manual(ora_fine_txt)
        durata_h = ore_da_orari(ora_inizio, ora_fine) if ora_inizio and ora_fine else 0

        if durata_h <= 0:
            st.warning("Inserire ora inizio e ora fine evento valide nel formato HH:MM.")
        else:
        ...
        with r2:
            if st.button("Salva turno", type="primary"):
                if data_turno is None:
                    st.error("Turno non salvato: devi inserire la data turno nel formato gg/mm/aaaa.")
                    st.stop()
